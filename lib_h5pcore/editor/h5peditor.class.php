...
class H5peditor {
  ...
  private $storage, $files_directory, $basePath, $h5pStorage;
   ...
   */
  function __construct($storage, $files_directory, $basePath, $h5pStorage = NULL) {
  ...
  public function processParameters($contentId, $newLibrary, $newParameters, $oldLibrary = NULL, $oldParameters = NULL) {
    ...
    foreach ($newLibraries as $library) {
      $libraryFull = $this->h5pStorage->h5pF->loadLibrary($library['machineName'], $library['majorVersion'], $library['minorVersion']);
      $librariesUsed[$library['machineName']]['library'] = $libraryFull;
      $librariesUsed[$library['machineName']]['preloaded'] = 1;
      $this->h5pStorage->getLibraryUsage($librariesUsed, $libraryFull);
    }

    $this->h5pStorage->h5pF->deleteLibraryUsage($contentId);
    $this->h5pStorage->h5pF->saveLibraryUsage($contentId, $librariesUsed);
    ...
    switch ($field->type) {
      ...
      case 'library':
        if (isset($params->library) && isset($params->params)) {
          ...
          // FIXME: Get Core directly, not via Storage
          $libraryData = $this->h5pStorage->h5pC->libraryFromString($params->library);
